...
Files generated by the `hn update` tool are placed in the `updated/` directory. They are named so that they continue the sequence described above. You may need to add leading 0's to these files since the `hn update` tool will not do this for you.
